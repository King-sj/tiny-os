...
- `src/bootpack.c` - C语言主程序，实现图形界面绘制
- `src/naskfunc.nas` - 汇编函数库，为C代码提供底层接口
- `src/sprintf.c` - 简化版sprintf实现
...
3. **ASMHEAD阶段**:
   ...
   - 进行实模式显存测试
   - 切换到保护模式
   - 设置GDT（数据段基址0，代码段基址0x280000）
4. **BOOTPACK阶段**: 执行C代码主程序，绘制桌面界面和字符显示
...
### 当前已解决的问题
- ✅ **GDT段基址配置**: 正确设置代码段基址为0x00280000
- ✅ **跳转地址计算**: 使用分段模式，跳转到段基址+偏移0
- ✅ **字体渲染**: 完整的ASCII字符集显示
- ✅ **内存管理**: 清晰的内存分区和地址映射
...
- **0x00280000**: C代码(bootpack)的实际入口地址
  - 0x8200 + 512字节 = 0x00280000
